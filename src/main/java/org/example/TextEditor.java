...
class TextEditor extends Frame implements ActionListener {
    private HashMap<String, Runnable> actionMap = new HashMap<String, Runnable>();
    ...
    private void handleCommand(String command) {
        ...
        if (action != null) {
            action.run();  // Executes the corresponding action
        } else {
...
//    }
    ...
    private void populateMappings(){
        actionMap.put("Exit", () -> System.exit(0));
        ...
        actionMap.put("Copy", () -> str = ta.getSelectedText());
        ...
        actionMap.put("Select All", () -> {
            String strText = ta.getText();
            int strLen = strText.length();
            ta.select(0, strLen);
        });

        actionMap.put("Time Stamp", () -> {
            String hms = getHMS();
            int loc = ta.getCaretPosition();
            ta.insert(hms, loc);
        });

        actionMap.put("About TextEditor", () -> {
